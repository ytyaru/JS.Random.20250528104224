...
//        return (v < this.min || this.max < v) ? v % this.length : v
        ...
        if (this.min<=v && v<=this.max) {return v}
            ...
            const L = this._signed ? this.length : this.#halfLen;
            if (this._signed) {return (this.max < v) ? (v % this.length) : (this.length - ((v*-1) % this.length))}
            else {return (this.max < v) ? (v % this.#halfLen) : (this.#halfLen - ((v*-1) % this.#halfLen))}
        }
            return 

        // unsignedでmaxを越えたら？
        if (!this._signed && this.max < v) {return v % this.length}
        // unsignedでminを下回ったら？
        if (!this._signed && v < this.min) {return this.length - ((v*-1) % this.length)}
        // singedでmaxを越えたら？
        if (this._signed && this.max < v) {return v % this.#halfLen}
        // singedでminを下回ったら？
        if (this._signed && v < this.min) {return this.#halfLen - ((v*-1) % this.#halfLen)}



        // unsignedなのに負数にしたら？　最大値にすべき。
        if (!this._signed && v < this.min) {
            const M = this.max;
            let V = v;
            while(M < V) {V=this.max + V;}
            return V;
        }
        // singedでmaxを越えたら？
        //if (this._signed && this.max < v) {return v % this.length}
        ...
        if (this._signed && v < this.min) {return this.#halfLen - ((v*-1) % this.#halfLen)}
        if (this._signed && v < this.min) {
            Math.abs(this.min) % this.length
            // -1 % 2^32  1%2^32
            return (Math.abs(this.min)-Math.abs(v)) % this.length
...
//        return v % this.length
    ...
    #overflow___(v) {
        ...
        if (!this._signed) {
            if (0<=v) {// 正数
                ...
                return 0===((v*-1)%this.length) ? 0 : this.length - ((v*-1) % this.length)
            }
        }
        if (!this._signed && 0<=v && (0===(v % this.length))) {return 0}
        if (!this._signed && v<0)
    }
    */
    #signedToUnsigned(v) {
    ...
    #overflow___(v) {
        const base = 256;
        const unsignedValue = 0; // 負数を正数に表記し直す。u8:-128〜127 0x7F(127),0x80(-128),...0xFF(-1)
        const figs = (this.#isB ? BigInt : Number).getFigureValuesOfBase(v,base);
        const bins = figs.slice(this.byteSize*-1); // 後ろからbyteSize分だけ取得する
        //const n = bins.reduce((s,v,i)=>s+(v*(base**i)),0);
        return bins.reduce((s,v,i)=>s+(v*((this.#isB ? BigInt(base) : base)**(this.#isB ? BigInt(i) : i))),0);
    }
    */
    ...
    */
    #overflow___(v) {
        if (this._signed) {
            if (v<0) {//
                //return (v*(this.#isB ? -1n : -1)) % this.#halfLen;
                //return this.#halfLen + ((v*(this.#isB ? -1n : -1)) % this.#halfLen);
//                return this.#halfLen + (v % this.#halfLen);
                const a = (v % this.#halfLen);
                return 0===a ? 0 : this.#halfLen + a;
            }
            else {
            ...
            if (v<0) {//
                const a = (v % this.length);
                return 0===a ? 0 : this.length + a;
//                return (v*(this.#isB ? -1n : -1)) % this.length;
                //return this.length + ((v*(this.#isB ? -1n : -1)) % this.length);
//                console.log(v, (v % this.length))
//                return this.length + (v % this.length);
    ...
    /*
    // !this._signed && 0<=v && (0===(v % this.length)) ? 0 : v<
    #overflow(v) {return (this.min<=v && v<=this.max) ? v : this.#overflowSU(v,(this._signed ? this.#halfLen : this.length))}
    //#overflow(v) {return (this.min<=v && v<=this.max) ? v : this.#overflowSU(v,(this._signed ? this.#halfLen : this.length))}
    // max超過 or min超過
    //#overflowSU(v,L) {return (this.max < v) ? (v % L) : (L - ((v*(this.#isB ? -1n : -1)) % L))}
    //#overflowSU(v,L) {console.log(v,L,this.max < v, (v % L), (L - ((v*(this.#isB ? -1n : -1)) % L)));return (this.max < v) ? (v % L) : (L - ((v*(this.#isB ? -1n : -1)) % L))}
    #overflowSU(v,L) {
        const V=(v*(this.#isB ? -1n : -1)); // 絶対値
        console.log(v,L,this.max < v, (v%L), ((0===(V%L)) ? 0 : (L - (V%L))));
        return (this.max < v) ? (v%L) : ((0===(V%L)) ? 0 : (L - (V%L)));
    }
    */
    ...
    //#overflowSU(v,L) {return (this.max < v) ? (v % L) : (L - ((v*(this.#isB ? -1n : -1)) % L))}
    /*
    #overflowSU(v,L) {// max超過 or min超過
        return this.#isB ? ((this.max < v) ? (v % L) : (L - ((v*-1n) % L))) : ((this.max < v) ? (v % L) : (L - ((v*-1) % L)));
        return (this.max < v) ? (v % L) : (L - ((v*-1) % L))
    }
    */
    //#overflowSU(v,L) {return (this.max < v) ? (v % L) : (L - ((v*-1) % L))} // max超過 or min超過
    /*
    #overflow(v) {
        if (this.min<=v && v<=this.max) {return v}
        else {return this.#overflowSU(v,(this._signed ? this.length : this.#halfLen))}
//            const L = this._signed ? this.length : this.#halfLen;
//            return this.#overflowSU(v,L)
//            if (this._signed) {return (this.max < v) ? (v % this.length) : (this.length - ((v*-1) % this.length))}
//            else {return (this.max < v) ? (v % this.#halfLen) : (this.#halfLen - ((v*-1) % this.#halfLen))}
//        }
    }
    */
    ...
    get max() {return this.#isB ? (this._signed ? ((2n**BigInt(this._bitSize))/2n)-1n : 2n**BigInt(this._bitSize)-1n) : (Math.floor((2**this._bitSize)/2)-1 : 2**this._bitSize-1)}
    //get max() {return this._signed ? (this.#isB ? ((2n**BigInt(this._bitSize))/2n)-1n : (Math.floor((2**this._bitSize)/2)-1)) : this.length + ((32 < this._bitSize) ? -1n : -1)}
    get max() {return this._signed ? (this.#isB ? ((2n**BigInt(this._bitSize))/2n)-1n : (Math.floor((2**this._bitSize)/2)-1)) : this.length + ((32 < this._bitSize) ? -1n : -1)}
    */
    /*
    get max() {
        const l = this._signed ? this.#halfLen : this.length;
        const o = this.#isB ? -1n : -1;
        return l + o;
    }
    */

    //get #length() {return 2n**this._bitSize-1}
    //get length() {return 2**this._bitSize-1}
    //get length() {return (32 < this._bitSize) ? 2n**BigInt(this._bitSize)-1n : 2**this._bitSize-1}
//    get length() {return (32 < this._bitSize) ? 2n**BigInt(this._bitSize) : 2**this._bitSize}
//    get min() {return (32 < this._bitSize) ? (this._signed ? (((-2n)**BigInt(this._bitSize))/2n : 0n)) : (this._signed ? (Math.floor(((-2)**this._bitSize)/2) : 0)) }
//    get max() {return (32 < this._bitSize) ? (this._signed ? ((2n**BigInt(this._bitSize))/2n)-1n : 2n**BigInt(this._bitSize)-1n) : (Math.floor((2**this._bitSize)/2)-1 : 2**this._bitSize-1)}
//    get max() {return this._signed ? () : this.length + ((32 < this._bitSize) ? -1n : -1)}
//    get min() {return this._signed ? Math.floor(((-2)**this._bitSize)/2) : 0}
//    get max() {return this._signed ? Math.floor((2**this._bitSize)/2)-1 : 2**this._bitSize-1}
//    get min() {return 'bigint'===typeof this.length ? this.#minB : this.#minN }
//    get max() {return 'bigint'===typeof this.length ? this.#maxB : this.#maxN }
    /*
    get min() {return (32 < this._bitSize) ? this.#minB : this.#minN }
    get max() {return (32 < this._bitSize) ? this.#maxB : this.#maxN }
    get #minN() {return this._signed ? Math.floor(((-2)**this._bitSize)/2) : 0}
    get #minB() {return this._signed ? ((-2n)**BigInt(this._bitSize))/2n : 0n}
    get #maxN() {return this._signed ? Math.floor((2**this._bitSize)/2)-1 : 2**this._bitSize-1}
    get #maxB() {return this._signed ? ((2n**BigInt(this._bitSize))/2n)-1n : 2n**BigInt(this._bitSize)-1n}
    */
    /*
    get(v) {
        if (52 < this._bitSize) {return parseInt(v)}
        else {return this._signed ? : 2**this._bitSize}
    ...
    static get(bitSize, signed=false) {
        if (52 < bitSize) {}
        else {return signed ? : 2**bitSize}
    }
    Int.get(bitSize, signed)
    */
}
class Seed {
