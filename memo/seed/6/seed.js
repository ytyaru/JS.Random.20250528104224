...
//    if (!(type instanceof IntType)) {throw new TypeError(`第二引数typeはIntTypeインスタンスであるべきです。`)}
//    if (type.signed) {type = IntType.get(type.bitSize, false);}
...
/*
String.fromInt = function(value, type, base, isU=false) {
    if (!(type instanceof IntType)) {throw new TypeError(`第二引数typeはIntTypeインスタンスであるべきです。`)}
    if (type.signed) {type = IntType.get(type.bitSize, false);}
    const v = type.convert(value); // unsigned型で強制変換する。
...
String.fromInt = function(value, base, isU=false, signed=false) {//数valueを基数baseとした文字列に変換して返す
    if ([value,base].every(v=>Number.isSafeInteger(v))) {// Number型
        if (signed
            ? ((-2)**(base-1)<=value && value<=2**(base-1)-1)
        ...
        if (signed
            ? ((-2n)**(base-1n)<=value && value<=(2n**(base-1n)-1n))
...
//    constructor(value=undefined, bitSize=32, signed=false, base=64, isBase64URL=false) {
    ...
    get s() {
        ...
        */
        /*
        const type = IntType.get(bitSize, false);
        const v = this._type.convert(this._v); // unsigned型で強制変換する。
        const figs = Number.getFigureValuesOfBase(v, this._base);
        const chars = String.getBaseChars(this._base, this._isBase64URL);
        return figs.map(f=>chars[f]).join('');
        */
