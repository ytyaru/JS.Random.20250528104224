...
class Real32 {//s32/u32な値を0以上1未満の実数にして返す
    static convert(n) {
        if (Number.isSafeInteger(n)) {
            ...
            else if (0x100000000<=n) {return (n & 0xFFFFFFFF)/0x100000000}
            else if (n<(-2)**31) {return ((n & 0xFFFFFFFF)+0x100000000)/0x100000000}
            //else if (n<(-2)**31) {return I32.s2u(n)(n & 0xFFFFFFFF)/0x100000000}
            // 範囲外 x<(-2)**31 && 2**32<=x
        }
        throw new TypeError(`引数nは(-2)**31〜2**32-1迄のNumber型な整数であるべきです。:${n}`,n)
    }
    static converts(ns) {
        //const rs = ns.map((n,i)=>{[...new Array(i+1)].map(_=>n=this.convert(n));return n;});
//        const rs = ns.map((n,i)=>{[...new Array(i+1)].map(_=>this.convert(n)));
//        return rs.reduce((s,v,i)=>s+v < 1 ? s+v : s,0);
//        const rs = this.ns.map((n,i)=>{[...new Array(i+1)].map(_=>n/=0x100000000);return n;});
//        return rs.reduce((s,v,i)=>s+v < 1 ? s+v : s,0);
...
class UintValues {
    ...
    get r() {return Real32.converts(this.ns);}
    /*
    get r() {
        const rs = this.ns.map((n,i)=>{[...new Array(i+1)].map(_=>n/=0x100000000);return n;});
        return rs.reduce((s,v,i)=>s+v < 1 ? s+v : s,0);
...
//    get r() {return Real32.convert(this.i)}
    *is(n=3) {for(let i=0; i<n; i++){yield this.i}}
    *rs(n=3) {for(let i=0; i<n; i++){yield this.r}}
}
...
class Xorshift128p {
    ...
    //get r() {return Real32.converts(this.ns);}
    get r() {return Real32.converts(this.ns.map(n=>n>>>0));}
    /*
    get r() {
        const ns = this.ns;
        const rs = ns.map((n,i)=>{[...new Array(i+1)].map(_=>n/=0x100000000);return n;});
