...
class Range {//連続有限整数範囲(指定した始点〜終点までの整数。前後の値差は必ず1であり連続している)
    ...
    static fromLength(length) {
        if (Number.isSafeInteger(length)) {
            if (1<length) {return new Range(0, length-1)}
                ...
                if (0===Math.abs(length%2)) {return new Range(H, (H*-1)-1)}// ゼロは正数として扱えば負数と同数になる
                else {return new Range(H, (H*-1))} // ゼロは正負どちら側でもない値として扱えば負数と同数になる
            }
        }
    ...
    static fromSpan(min, max) {return new Range(min, max)}
    static of(...args) {
             if (0===args.length) {return new Range(1, 6)}
...
//    constructor(min, max) {this.v = [min,max];}
    constructor(...args) {
        /*
        this.v = [min,max];
             if (0===args.length) {this.v = [1, 6]}
        else if (1===args.length) {this.v = this.#getMinMax(args[0])}
        else if (2===args.length) {this.v = [...args]}
        else {throw new TypeError(`引数は(length)/(min,max)の2パターンのみ有効です。`)}
        */
        const L = args.length;
    ...
    #getMinMax(length) {
        if (Number.isSafeInteger(length)) {
            if (1<length) {return [0, length-1]}
                ...
                console.log(H, length, length/2, Math.floor(length/2), Math.trunc(length/2))
...
//        return v / this.l;
        if (this.within(v)) {return v / this.l;}
    }
    #isValid(v) {if(Number.isSafeInteger(v)){return true}else{throw new TypeError(`Number.isSafeInteger(v)であるべきです。`)}}
    #isNum(v){if('number'===typeof v){return true}else{throw new TypeError(`Number型であるべきです。`)}}
    #isRate(v){if(!this.#isNum(v) || r<0 || 1<r){throw new RangeError(`0〜1迄の少数であるべきです。`)}else{return true}}
    within(v){if(!this.#isNum(v) || v<this.m || this.M<v){throw new RangeError(`0〜1迄の少数であるべきです。`)}else{return true}}
}
...
class Randomable {// Number(整数(0〜length-1 / min〜max)/小数(0≦r<1)), Booleanを返す
    ...
    p(r=0.5) {// 指定した確率で真を返す。r:閾値
        //if ('number'!==typeof r || r<0 || 1<r) {throw new TypeError(`引数は0〜1迄の少数であるべきです。`)}
//        if ('number'!==typeof) {throw new TypeError(`引数は0〜1迄の少数であるべきです。範囲外なら自動的に0か1とします。`)}
