...
class U32 {
    static toS32(u32) {
        if (0<=u32 && u32<=0xFFFFFFFF) {
            
        } else {throw new TypeError(`引数u32は0〜0xFFFFFFFF迄のNumber型な整数であるべきです。`)}
    }
    static r(N) {return this.s2u(N)/0x100000000}
    static rs(ns) {
...
class Real32 {//s32/u32な値を0以上1未満の実数にして返す
    static convert(n) {
        ...
        throw new TypeError(`引数nは(-2)**31〜2**32-1迄のNumber型な整数であるべきです。`)
    }
    static converts(ns) {
