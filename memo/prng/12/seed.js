...
BigInt.random = function(bitSize) {// bitSize=128の時はu128の乱数を返す。0〜2**128-1。
    const B = 2; // base 基数
    const bin = Array(bitSize).fill().map(()=>Math.floor(Math.random()*B).toString(B)).join('');
    return BigInt(`0b${bin}`); // 0b0011等
}
Number.randomInt = function(bitSize=32) {
    if (Number.isSafeInteger(bitSize) && 0<bitSize && bitSize<53) {return Math.floor(Math.random()*(2**bitSize))}
    else {throw new TypeError(`引数bitSizeは1以上53未満であるべきです。`)}
}
//Number.randomReal = function(bitSize=32) {return Math.random()}// this.randomInt(bitSize) / (2**bitSize) は冗長
...
//        this._base = base;
    ...
    get typeName() {return `${this.type.bitTypeName}b${this._base}${this._isBase64URL ? 'u' : ''}`}
    ...
    get v() {return `${this.typeName}:${this.s}`}
    //get v() {return `${this.type.bitTypeName}b${this._base}${this._isBase64URL ? 'u' : ''}:${this.s}`}
    toString(base, isU) {return String.fromInt(this._v, this._type.bitSize, base ? base : this._base, isU ? isU : this._isBase64URL);}
...
//            console.log(parseInt(bitSizeStr), value, base, isBase64URL)
        ...
        else if (arg instanceof IntType) {
            const [type, base, isBase64URL] = [arg, 64, true];
//            const value = type._isB ? Math.random()*(2**type.bitSize) : Math.random()*(2n**BigInt(type.bitSize))
//            const value = type._isB ? Math.floor(Math.random()*(2**type.bitSize)) : BigInt.random(type.bitSize);
//            const R = type._isB ? Number.randomInt : BigInt.random;
//            const value = R(type.bitSize);
            const value = (type._isB ? BigInt.random : Number.randomInt)(type.bitSize);
            console.log(value, type, base, isBase64URL);
            return new Seed(value, type, base, isBase64URL);
        }
