...
//            if (undefined!==v) {return Object.seal(v)}// 構成不可。
        }
    ...
    fromWeights(weights) {
        //if (Array.isArray(weights) && weights.every(w=>Number.isSafeInteger(w) && 0<=w) && !weights.every(w=>w===0)) {return weights.map((v,i)=>({i:i, w:v}))}
    ...
    fromMix(array) {
        //if (Array.isArray(array) && array.every(a=>(Number.isSafeInteger(a) && 0<=a)||(Array.isArray(a) && 2===a.length && a.every(v=>Number.isSafeInteger(v) && 0<=v))) && !array.every(a=>a===0)) {return array.map((v,i)=>Array.isArray(v) ? ({i:i, w:v}) : ({i:i, w:v[0], q:v[1]}))}
    ...
    fromArray(array) {
        //if (Array.isArray(array) && array.every(a=>Array.isArray(a) && 2<=a.length && a.every(v=>Number.isSafeInteger(v) && 0<=v)) && !array.every(a=>a[1]===0) && (new Set(array.map(a=>a[0]))).size===array.length) {return array.map((v,i)=>2===v.length ? ({i:v[0], w:v[1]}) : ({i:v[0], w:v[1], q:v[2]}))}
    ...
    fromObjects(objs) {
        if (objs.every(o=>Candidate.#isObj(o) && 'i w'.split(' ').every(o=>o.hasOwnProperty(n) && Number.isSafeInteger(o[n])) && 0<=o[k]) && !objs.every(v=>v.w===0)) {
            objs.map((v,i)=>{
    ...
    constructor(...args) {// ([w,...]) / ([w,...],true) / ([w,[w,q]],...) / ([[i,w],[i,w,q],...]) / ([{i:i,w:w},{i:i,w:w,q:q},...])
        ...
        this._isAllFinite = isFinite || this._v.findIndex(o=>o.hasOwnProperty('q'));
        this._hasFinite = !this._isAllInfinity && !this._isAllFinite;

        this._backupLength = this._v.length;
        this._backupV = this._hasFinite ? JSON.parse(JSON.stringify(this._v)) : null;
        this._backupFiniteds = this._hasFinite ? this.finiteds : null;

        this._backupFiniteds = this.finiteds;
        this._vs = {
            infiniteds: this.infiniteds,
            finiteds: this.finiteds,
            backupFiniteds: this.finiteds,
        };
        this._isAutoReset = 'function'===args[args.length-1] ? args[args.length-1] : (cand)=>cand.isSoldOut;
    }
    ...
    //get isFullStock() {return this.isInfinity ? 0===this.banneds.length : 0===this.notExistFiniteds.length && 0===this.banneds.length;}//有限数の品があるとき品数は初期値以上であるか
    ...
    get weightSum() {return this._candidates.reduce((s,o,i)=>s+o.w, 0)}
    // 部分候補の取得
    ...
    // 品切れしたものを補充し、weight=0でBANしたものを元に戻す
    autoReset() {if (this._isAutoReset(this)) {this.reset()}}// 条件に合致した場合、品切れしたものを補充する
    autoReQ() {if (this._isAutoReQ(this)) {this.reQ()}}// 条件に合致した場合、品切れしたものを補充する
    autoReW() {if (this._isAutoReW(this)) {this.reW()}}// 条件に合致した場合、禁止したものを解禁する
    ...
    reQ() {//replenish 補充する。quantityを初期値に戻す。
        for (let i=0; i<this._backupV.length; i++) {
            this._v[i].q = this._backupV[i].q;
        }
    }
    ...
    /*
    resetWeights() {
        if (this._backupLength===this._v.length) {// this._vのうちq:0のobjectを削除せず保持し続ける場合
            this.banneds.map(b=>{
                const I = this._v.findIndex(x=>x.i===b.i);
                this._v[I].w = this._backupV[I].w;
            });
            for (let i=0; i<this._backupV.length; i++) {
                if (0===this._v[i].w) {this._v[i].w = this._backupV[i].w;}
            }
        } else {// this._vのうちq:0のobjectを削除して保持しない場合

        }
    }
//    replenish() {this._vs.finiteds = JSON.parse(JSON.stringify(this._vs.backupFiniteds));}
    replenish() {// 補充する。qを初期値に戻す。
        if (this._isAllInfinity) {return}
        if (this._backupLength===this._v.length) {// this._vのうちq:0のobjectを削除せず保持し続ける場合
            for (let i=0; i<this._backupFiniteds.length; i++) {
                const b = this._backupFiniteds[i];
                const I = this._v.findIndex(V=>V.i===b.i);
                this._v[I].q = b.q;
            }
        } else {// this._vのうちq:0のobjectを削除して保持しない場合
            this._vs.finiteds = JSON.parse(JSON.stringify(this._vs.backupFiniteds));
            this._v = [...this._vs.infiniteds, this._vs.finiteds];
            this._v.sort((a,b)=>a.i-b.i);
        }
        if (this._backupFiniteds)

        // this._vのうちq:0のobjectを削除せず保持し続ける場合
        for (let i=0; i<this._backupFiniteds.length; i++) {
            const b = this._backupFiniteds[i];
            const I = this._v.findIndex(V=>V.i===b.i);
            this._v[I].q = b.q;
        }
        // this._vのうちq:0のobjectを削除して保持しない場合
//        this._vs.finiteds = JSON.parse(JSON.stringify(this._vs.backupFiniteds));
//        this._v = [...this._vs.infiniteds, this._vs.finiteds];
//        this._v.sort((a,b)=>a.i-b.i);
...
class Lottery {// 抽選（候補となる配列からランダムに抽出する。無限／有限、割合）
    constructor(candidates, random) {
        ...
        this._cand = Candidate.of(candidates, isFinite);
    }
    get prng() {return this._prng}
...
class FiniteLottery {
    constructor(candidates, random) {
        this._backup = JSON.parse(JSON.stringify(this._candidates));
    }
    replenish() {
        this._candidates = JSON.parse(JSON.stringify(this._backup));
    }
}
class Lottery {// 抽選（候補となる配列からランダムに抽出する。無限／有限、割合）
    constructor(candidates, random) {
