...
// rate:比率|割合(1時間あたり等), ratio:二つの比(1:2), proportion:全体に対する割合,比率, percentage:100を基準とした割合,比率, probability:確率
Boolean.random = function(rate=0.5) {
    return Is.safeNum(R) 
        ? (0===R) ? false
        : ((1<=R)) ? true
        : (RndReal.V()<=R)
        : (()=>{throw new TypeError('引数はNaNを除いたNumber型であるべきです。0以下なら偽、1以上なら真、0.3なら30%で真を返す。')})();
}
Number.random = function(...args) {
//    0===args.length
//    1===args.length && Is.N(args[0])
//    2===args.length && Is.Ps(args) && args[0] < args[1]
    const R = RndReal.V();
    if (0===args.length) {return R}
    else if (1===args.length && Is.N(args[0])) {return Math.floor(R * args[0])}
    else if (2===args.length && Is.Ps(args) && args[0] < args[1]) {return Math.floor(R * (args[1]-args[0]+1)) + args[0]}
    else {throw new TypeError(`引数は0〜2個であるべきです。1個なら自然数、2個なら(小,大)。小,大は正数であり差が1以上のみ有効です。`)}
}

class RndBool {
    //static rate(R) { return (R <= 0) ? false : ((1 <= R) ? true : (RndReal.V() <= R)); }
    static rate(R) { return Is.safeNum(R) 
        ? (0===R) ? false
        : ((1<=R)) ? true
        : (RndReal.V()<=R)
...
class RndInt {
    static pieces(R) {return Math.floor(RndReal.V() * R)} // num(), numberOf(), pieces(), quantity()
...
// RndInt.pieces(5)
// RndInt.quantity(5)
// RndInt.of(1,2,3,4,5) // 引数のうちいずれか一つを返す。これはArray.prototype.draw()で実装すればいいので省略する。
...
if (!('chooseIndex' in Array.prototype)){Array.prototype.chooseIndex = function(A) {// 0〜要素数-1の正数を返す。
    return ('function'===A)
        ? Lottery.draw(this.map((v,i)=>fn(v,i)))
        : ((Is.Ps(A) && Ws.length===this.length)
            ? Lottery.draw(A)
            : RndInt.pieces(this.length));
}}
if (!('draw' in Array.prototype)){Array.prototype.draw= function(A) {// 0〜要素数-1位置にある要素を返す。要素は配列から削除する。
    this.chooseIndex(A)
    const i = ('function'===A)
        ? Lottery.draw(this.map((v,i)=>fn(v,i)))
        : ((Is.Ps(A) && Ws.length===this.length)
            ? Lottery.draw(A)
            : RndInt.pieces(this.length));
    const vs = this.splice(i, 1); // i番目の要素を1つ抽出して新しい配列として返す。自身の要素からそれを削除する。
    return vs[0];
    /*
...
if (!('choose' in Array.prototype)){Array.prototype.toDrawn= function(A) {// 0〜要素数-1位置にある要素を返す。要素は削除せず。
    return this[('function'===A)
        ? Lottery.draw(this.map((v,i)=>fn(v,i)))
        : ((Is.Ps(A) && Ws.length===this.length)
            ? Lottery.draw(A)
...
/*
if (!('toDrawn' in Array.prototype)){Array.prototype.toDrawn= function(A) {// 0〜要素数-1位置にある要素を返す。要素は削除せず。
    return this[('function'===A)
        ? Lottery.draw(this.map((v,i)=>fn(v,i)))
        : ((Is.Ps(A) && Ws.length===this.length)
            ? Lottery.draw(A)
...
if (!('choice' in Array.prototype)){Array.prototype.choice = function(Ws) {
    if (undefined===Ws) {return this[RndInt.pieces(this.length)]}
...
//    [...new Array(N)].map((_,i)=>i)
    ...
    //if (undefined===Ws) {return this[RndInt.pieces(this.length)]}
    if (undefined===Ws) {return [...new Array(N)].map((_,i)=>this[RndInt.pieces(this.length)])}
...
Number.draw = function(...args) {// 抽選する(正数を返す)
    if ('function'!==typeof args[args.length-1]) {throw new TypeError(`最後の引数は関数であるべきです。添字を引数とし重みを返します。重みは正数であり0なら当選せず1以上なら重み合計で割った確率で当選します。`)}
    if (!(2===args.length && Is.N(args[0]))
    ||  !(3===args.length && Is.P(args[0]) && Is.P(args[1]) && args[0] < args[1])) {throw new TypeError(`引数は2〜3個であるべきです。2個なら(個,重)、3個なら(開,終,重)。個は自然数、開終は正数、重は正数を返す関数であり(開始,終了,値,添字)が引数です。重は重みのことで当選確率を算出します。当選確率＝個別重み÷全重み。例えば Number.draw(5, (s,e,v,i)=>0===v%2 ? 2 : 1); なら0〜4までの整数を一つ返し、その当選確率は偶数(0,2,4)がそれぞれ2/8、奇数(1,3)がそれぞれ1/8です。8=2+1+2+1+2。`)}
    const S = 2===args.length ? 0 : args[0];
    const E = 2===args.length ? args[0]-1 : args[1];
//    const L = E-S+1;
//    const Cs = Array.seq(L,S); // 候補[S,...,E]
    const Cs = Sequence.make(S,E); // 候補[S,...,E]
    const Ws = Lottery.draw(Cs.map((S,E,c,i)=>fn(S,E,c,i))); // 重み配列
//    const WS = Ws.reduce((s,v,i)=>s+v,0); // 重み合計
    return Lottery.draw(Ws);
    /*
    if ('function'!==typeof args[args.length-1]) {throw new TypeError(`最後の引数は関数であるべきです。添字を引数とし重みを返します。重みは正数であり0なら当選せず1以上なら重み合計で割った確率で当選します。`)}
    if (2===args.length && Is.N(args[0])) {return Math.floor(RndReal.V() * args[0])}
    else if (3===args.length && Is.P(args[0]) && Is.P(args[1]) && args[0] < args[1]) {return Math.floor(RndReal.V() * (args[1]-args[0]+1)) + args[0]}
    else {throw new TypeError(`引数は2〜3個であるべきです。2個なら(個,重)、3個なら(開,終,重)。個は自然数、開終は正数、重は正数配列です。重は重み配列のことで当選確率を算出します。`)}
    */
}
Number.draws = function(...args) {// 複数回抽選する(正数配列を返す)
    const fn = args[args.length-1];
    if ('function'!==typeof fn) {throw new TypeError(`最後の引数は関数であるべきです。添字を引数とし[重み,当選上限回数]の配列を返します。重みは正数であり0なら当選せず1以上なら重み合計で割った確率で当選します。当選上限回数はその数が当選できる最大回数です。`)}
    const S = 2===args.length ? 0 : args[0];
    const E = 2===args.length ? args[0]-1 : args[1];
    const Cs = Array.seq(E-S+1); // [S,...,E]
//    const Cs = [...new Array(E-S+1)].map((_,i)=>i)
    const WS = Cs.reduce((s,v,i)=>s+fn(v,i)[0],0); // 重み合計
    args[1](i)
    if (2===args.length && Is.N(args[0])) {return Math.floor(RndReal.V() * args[0])}
    else if (3===args.length && Is.P(args[0]) && Is.P(args[1]) && args[0] < args[1]) {return Math.floor(RndReal.V() * (args[1]-args[0]+1)) + args[0]}
    else {throw new TypeError(`引数は2〜3個であるべきです。2個なら(個,重)、3個なら(開,終,重)。個は自然数、開終は正数、重は正数配列です。重は重み配列のことで当選確率を算出します。`)}
